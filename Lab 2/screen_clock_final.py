...
while True:
    ...
    q1 = "Today is a good day!"
    q2 = "Smile : )"
    q3 = "Good Luck ~"
    q4 = "Sleep well and relax !"
    q5 = "Good Morning!"
    q6 = "Sweet Dream :)"
    q7 = "Nice Job!!"
    q8 = "Get some rest !"
    ...
    quote_color = "#"+''.join([random.choice('0123456789ABCDEF') for i in range(6)])
    ...
    if nowhours in range(0,6): #midnight
        ...
        draw.text((6,80), q6, font=font3, fill=quote_color)
    elif nowhours in range(6,13): #morning
        draw.text((x,y), nowtime, font=fontTime, fill="#f0e630")
        if nowhours in range(6,10):
            draw.text((6,80), q5, font=font3, fill=quote_color)
        elif nowhours in range (10,13):
            draw.text((6,80), q1, font=font3, fill=quote_color)

    elif nowhours in range(13,18): #afternoon
        draw.text((x,y), nowtime, font=fontTime, fill="#f08d30")
        if nowhours in range(13,15):
            draw.text((6,80), q3, font=font3, fill=quote_color)
        elif nowhours in range(15,18):
            draw.text((6,80), q7, font=font3, fill=quote_color)
    elif nowhours in range(18,24): #night
        draw.text((x,y), nowtime, font=fontTime, fill="#237bd9")
        if nowhours in range(18,21):
            draw.text((6,80), q8, font=font3, fill=quote_color)
        elif nowhours in range(21,24):
            draw.text((6,80), q4, font=font3, fill=quote_color)
    ...
    if not buttonA.value:
        x1, y1 = 6, 113
        draw.text((x1,y1), "Sunrise", font=font1, fill="#FFFF9C")
        y += font.getsize(nowtime)[1]
        draw.text((65, 110), sunrise, font=fontSunt, fill="#f5da2c")
        image1 = Image.open("sunrise_f.png")
        moon = image1.convert('RGB')
        disp.image(moon)
    if not buttonB.value:
        x2, y2 = 118, 113
        draw.text((x2,y2), "Sunset", font=font1, fill="#FFFF9C")
        y += font.getsize(sunrise)[1]
        draw.text((175, 110), sunset, font=fontSunt, fill="#f57c2c")
        image1 = Image.open("sunset_f.png")
        sun = image1.convert('RGB')
        disp.image(sun)
    # Display image.
